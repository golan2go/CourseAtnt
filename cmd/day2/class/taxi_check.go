...
const rootDir = "/Users/ig2258/Desktop/GoCourseResources/taxi"

func main() {
	start := time.Now()
	serialHandling()
	duration := time.Since(start)
	fmt.Printf("serialHandling: %v\n", duration)

	start = time.Now()
	parallelHandling()
	duration = time.Since(start)
	fmt.Printf("parallelHandling: %v\n", duration)
...
func parallelHandling() {
	sigs := readResultsFile()
	ch := goAll(sigs)
...
func goAll(sigs map[string]string) chan Job {
	ch := make(chan Job)
	for fileName, sha := range sigs {
		go asyncFileSig(fileName, sha, ch)
	}
	return ch
}

func asyncFileSig(fileName string, expectedSha string, ch chan Job) {
	sig, err := fileSig(path.Join(rootDir, fileName+".bz2"))
	if err != nil {
		ch <- Job{
			FileName: fileName,
			Error:    err,
...
func readResultsFile() map[string]string {
	file, err := os.Open(path.Join(rootDir, "sha256sum.txt"))
	if err != nil {
		log.Fatalf("error: %s", err)
	}
	...
	signatures, err := parseSigFile(file)
	if err != nil {
		log.Fatalf("error: %s", err)
	}
